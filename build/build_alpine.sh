...
PYEOF
    ...
    local data_files="gpu_disambiguation_rules.txt gpu_amd_cu_vram_rules.txt gpu_nvidia_literal_rules.txt gpu_nvidia_vram_rules.txt"
    for s in $scripts; do
        if [ -f "$SRC_DIR/$s" ]; then
    ...
    for f in $data_files; do
        if [ -f "$SRC_DIR/$f" ]; then
            cp "$SRC_DIR/$f" "$ovl_dir/opt/pcinfo/$f"
            log "  Installed: $f"
        else
            warn "  Missing source: $SRC_DIR/$f"
        fi
    done
